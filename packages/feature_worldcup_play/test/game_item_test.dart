...
void main() {
  ...
  const itemA = WorldCupItemModel(
    1,
    'assets/sample/female/aespa_carina.jpg',
    'A',
    -1,
  );
  const itemB = WorldCupItemModel(
    2,
  ...
  testWidgets('실제 WorldCupGame 위젯에서 여러 라운드에 걸쳐 매 대결마다 탭이 정상 동작한다 '
      '(setGame()이 매번 선택 상태를 초기화하는지 확인)', (WidgetTester tester) async {
    ...
    final items = [
      ...
      const WorldCupItemModel(
        4,
        'assets/sample/male/and2ble_yujin.jpg',
        'D',
        102,
      ),
    ...
    for (var match = 0; match < 6; match++) {
      ...
      expect(
        transitions.every(
          (transition) => transition.position.value == Offset.zero,
        ),
        isTrue,
