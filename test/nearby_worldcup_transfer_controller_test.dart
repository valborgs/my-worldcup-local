...
void main() {
  ...
  test('발견 기기 목록, 인증 코드, 수락 상태와 중복 전송 방지를 관리한다', () async {
    ...
    await _flush();
    expect(controller.endpoints.map((item) => item.name), [
      'Android',
      'iPhone',
    ]);
  ...
  test('연결 요청을 명시적으로 거절하고 리소스를 정리한다', () async {
    ...
    expect(
      controller.needsConnectionDecision,
      isFalse,
  ...
  test('패키지 생성 중 취소하면 생성 완료 후 전송을 다시 시작하지 않는다', () async {
    final gateway = _FakeNearbyGateway();
    final packageGateway = _FakePackageGateway(temporaryDirectory)
      ..createCompleter = Completer<String>();
    final controller = NearbyWorldCupTransferController.sender(
      gateway: gateway,
      packageGateway: packageGateway,
      worldCup: _worldCup,
    ...
    packageGateway.createCompleter!.complete(package.path);
    await _flush();
...
class _FakePackageGateway implements WorldCupPackagePort {
  ...
  Future<String> createPackage(WorldCupModel model) async {
    ...
    return (File(
      '${directory.path}/send.myworldcup',
