...
void main() {
  testWidgets('열린 온보딩은 언어가 바뀌면 제목, 본문, 버튼과 다음 페이지를 갱신한다', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(tester.state(find.byType(HelpScreen)), same(previousState));
    expect(find.text('Create a World Cup: Step 1'), findsOneWidget);
    expect(find.text('Skip'), findsOneWidget);
    expect(find.bySemanticsLabel(RegExp('Page 2 of 5')), findsOneWidget);
    await tester.tap(find.text('Next'));
    await tester.pumpAndSettle();
    expect(find.text('Create a World Cup: Step 2'), findsOneWidget);
    expect(find.bySemanticsLabel(RegExp('Page 3 of 5')), findsOneWidget);
    expect(tester.takeException(), isNull);
    semantics.dispose();
  });

  for (final locale in ['ja', 'en']) {
    test('$locale 리소스에 누락, 원문 잔류, placeholder 변경이 없다', () {
      Map<String, dynamic> read(String locale) => jsonDecode(
        File('packages/worldcup_ui_kit/lib/l10n/app_$locale.arb')
            .readAsStringSync(),
      ) as Map<String, dynamic>;
      final ko = read('ko');
      final ja = read(locale);
      ...
      for (final key in keys) {
        ...
        if (locale == 'en') {
          expect(RegExp('[ぁ-んァ-ン一-龯]').hasMatch(text), isFalse, reason: key);
        }
  ...
  for (final ja in [AppLocalizationsJa(), AppLocalizationsEn()]) {
    test('${ja.localeName} 샘플 매핑은 모든 후보를 번역하고 사용자 콘텐츠는 보존한다', () async {
